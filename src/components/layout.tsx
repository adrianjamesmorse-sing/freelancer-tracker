    ...
    <>
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(159,135,104,0.10),transparent_24%),radial-gradient(circle_at_top_right,rgba(127,142,108,0.10),transparent_22%),linear-gradient(180deg,#fbf8f2_0%,#f4efe6_100%)] text-stone-900">
        ...
        <div className="lg:flex lg:min-h-[calc(100vh-50px)]">
          {mobileOpen ? <button className="fixed inset-0 z-30 bg-stone-900/20 backdrop-blur-sm lg:hidden" aria-label="Close navigation" onClick={() => setMobileOpen(false)} /> : null}
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      [
                        ...
                        isActive ? 'bg-[#e6dccb] text-brand-800 shadow-sm' : 'text-stone-600 hover:bg-[#efe7da] hover:text-stone-900',
      ...
      >
        <div className="space-y-3">
          {notificationPreview.map((notification) => (
            <div key={notification.id} className="rounded-[22px] border border-stone-200 bg-white/80 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  {notification.subject ? <div className="font-medium text-stone-900">{notification.subject}</div> : null}
                  ...
                  {notification.recipientsPreview?.length ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {notification.recipientsPreview.map((recipient) => (
                        <span key={recipient} className="inline-flex rounded-full border border-stone-200 bg-[#f8f4ec] px-3 py-1 text-xs text-stone-600">
                          {recipient}
                        </span>
                      ))}
                    </div>
